...
static Expr *parseTerm( char *tok, FILE *fp )
{
  ...
  if ( tok[ 0 ] == '"' ) {
    ...
    return makeLiteral( strcpy( str, tok ) );
        ...
        char *str = (char *) malloc( strlen( tok ) + 1 );
        return makeAssignment(strcpy(str, tok));
    }
    else if (tok[0] == '(') {
...
Stmt *parseStmt( char *tok, FILE *fp )
{
  ...
  if ( strcmp( tok, "print" ) == 0 ) {
    ...
    char *varName = (char *) malloc( strlen( tok ) + 1 );
    Expr *name = makeLiteral(strcpy(varName, tok));
    if (strcmp(expectToken(tok, fp), "=") != 0)
        syntaxError();
    Expr *value = parseExpr(expectToken(tok, fp), fp);
    Expr *arg = makeAssignmentExpr(name, value);
    requireToken(";", fp);
    return makeAssignmentStmt(arg);
  }
  else if (strcmp(tok, "if") == 0) {
    Expr *cond = parseTerm(expectToken(tok, fp), fp);
